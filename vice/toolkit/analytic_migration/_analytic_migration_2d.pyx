...
from libc.math cimport NAN, sqrt, atanh, log
cimport cython
...
cpdef (double, double) sqrt_migration_2d(migration_star_2d* star, double time):
	"""
...
cpdef (double, double) linear_migration_2d(migration_star_2d* star, double time):
	"""
	"""
	cdef double x = (time - star.t_birth) / (star.t_end - star.t_birth)
...
@staticmethod
cdef double f_initial_default(double[::1] r_bins, int zone, double t, int n):
	"""
...
cdef double f_final_liam(double[::1] r_bins, int zone, double t, int n):
	"""
	The default final radius function. Returns the radius of the star at time t.
	"""
	cdef double r_low = r_bins[zone]
	cdef double r_high = r_bins[zone + 1]

	cdef double R = rand_range(r_low, r_high)
	cdef double z = rand_sec2()
...
cdef class c_analytic_migration_2d:
	...
	def __cinit__(self, radbins, f_initial, f_final, f_migration,
		...
		Boundary conditions
		- reflect
			The particle is reflected off of the boundaries
		- absorb
			The particle is absorbed into the wall
		- reflect_final
			The final position of the particle is reflected
		- absorb_final
			The final position of the particle, if too large or small, is set to the boundary.
		"""
		self.dt = dt
		self.t_end = t_end
		...
		self.alloc_arrays()
		self.init_radii(f_initial, f_final)
	...
	cdef call(self, int zone, double tform, double time, int n=0):
		...
		if self.write:
			self.write_migration(f"{zone},{time_int},{n},{time},{R_new},{bin_id}\n")
	...
	cpdef void init_radii(self, f_ini, f_fin):
		...
		for n in range(self.n_stars):
			for zone in range(self.n_zones):
				for i in range(self.n_t):
					...
					r_f, z_f = f_fin(self._radial_bins, zone, self.dt * i, n)
