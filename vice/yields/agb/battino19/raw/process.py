...
def read_battino(file_path, elements):
	"""Reads in the tables from either B19 or B21"""
	df =  read_csv(file_path, header=True, sep=r"\s+", comment="#")
	df = sum_isotopes(df, iso_key="isotope")
	df = sort_battino_eles(df, elements)

	assert df["Isotopes"] == elements, f"Elements mismatch {df['Isotopes']} != {elements}"
...
def sum_isotopes(data, iso_key="Isotopes", val_columns=None):
	"""
	...
	if val_columns is None:
		val_columns = data.keys() - {iso_key}

	first_columns = data.keys() - set(val_columns)
	...
	for i in range(N):
		...
		if ele not in summed_data["Isotopes"]:
			for k in val_columns:
				if k != iso_key:
					summed_data[k].append(float(data[k][i]))
			for k in first_columns:
				if k != iso_key:
					summed_data[k].append(float(data[k][i]))
